...
public class ScmUtil {
    ...
    public static Long nextCode(Long pid, List<Long> idList) {
        if (pid == null) {
            throw new ScmException(BaseErrorCode.PARAM, "pid不能为空");
        }
        ...
        throw new ScmException(BaseErrorCode.PARAM, "数据异常");
    }
     ...
     * @return 树的根节点列表
     */
    public static <T extends ScmTreeNode> List<T> listToTree(List<T> list) {
    ...
    public static <T extends ScmTreeNode> List<T> treeToList(List<T> itemList) {
        List<T> rootList = new ArrayList<>();
        addChildren(rootList, itemList);
        return rootList;
    }

    public static <T extends ScmTreeNode> List<T> treeToList(T item) {
        List<T> rootList = new ArrayList<>();
        rootList.add(item);
    ...
    private static <T extends ScmTreeNode> void addChildren(List<T> rootList, List<T> children) {
    ...
    public static String composeUrl(String domain, String api) {
        String url;
        if (ScmUtil.stringLast(domain).equals("/")) {
            if (ScmUtil.stringFirst(domain).equals("/")) {
                url = ScmUtil.cutLast(domain) + api;
            } else {
                url = domain + api;
            }
        } else {
    ...
    private static boolean loadJavaSource(Class<?> clazz) throws IOException {
        String moduleSourceDir = ScmUtil.findClassDirPath(clazz);
    ...
    public static Double findStepValue(TreeMap<Integer, Double> config, Double defaultValue, Integer rechargeAmount) {
        ...
        if (config != null) {
            ...
            if (defaultValue == null) {
                nowValue = defaultValueOpt.orElseThrow(() -> new ScmException(BaseErrorCode.CUSTOM, "错误的平台充值配置"));
            } else {
    ...
    public static void validate(Object req, Class<?>... groups) {
        ...
        if (CollUtil.isNotEmpty(validate)) {
            throw new ScmException(BaseErrorCode.PARAM.getCode(), validate.stream().map(ConstraintViolation::getMessage).collect(Collectors.joining(";\n")));
        }
    }
