 ...
 * @ext fill / reload / 执行
 * @ext String
 * @ext JobMeta meta, List<ExecuteLogDTO> locked
 * @ext 调 service 业务方法，回写 msg / bizSourceId
 * @ext fill 路径结束后发 JOB_CONTEXT_CLEAR
 */
@Slf4j
public abstract class BaseJob {
     ...
     */
    public abstract List<com.newzkl.platform.base.common.ddd.model.dto.ExecuteLogDTO> fill(String jobName);
     ...
     * @param jobBody 业务执行闭包 (locked → processed)，由 JobAspect 通过 pjp.proceed 提供 / JobInvoker 反射 raw target 提供
     * @return processed 列表 (A/D 路径) / 空列表 (B/C 路径)
     */
    public List<com.newzkl.platform.base.common.ddd.model.dto.ExecuteLogDTO> secondHandleProcess(JobContext.JobMeta meta,
                                                                                                 SecondLevelJobParam param,
                                                                                                 Function<List<com.newzkl.platform.base.common.ddd.model.dto.ExecuteLogDTO>, List<com.newzkl.platform.base.common.ddd.model.dto.ExecuteLogDTO>> jobBody) {
        String jobName = meta.getJobName();

        // 路径 B: fill（MQ clear 由 JobAspect 发送）
        if (param instanceof AssetJobParam a && a.isFillExecuteLog()) {
            List<com.newzkl.platform.base.common.ddd.model.dto.ExecuteLogDTO> expected = fill(jobName);
        ...
        List<com.newzkl.platform.base.common.ddd.model.dto.ExecuteLogDTO> pending = executeLogDomain.listPendingBefore(jobName, endTime, param.getExecuteLogIds());
        ...
        if (reloadPath) {
            ...
            for (com.newzkl.platform.base.common.ddd.model.dto.ExecuteLogDTO e : pending) {
        ...
        List<com.newzkl.platform.base.common.ddd.model.dto.ExecuteLogDTO> locked = pending.stream()
        ...
        List<com.newzkl.platform.base.common.ddd.model.dto.ExecuteLogDTO> processed = jobBody.apply(locked);
