...
public class SpuRepositoryImpl implements SpuRepository {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void refreshSpu(List<Long> spuId) {
        if (CollUtil.isEmpty(spuId)) {
            return;
        }
        for (Long id : spuId) {
            List<SkuDO> skuList = skuDAO.selectList(
                    new BaseLambdaQueryWrapper<SkuDO>().eq(SkuDO::getSpuId, id));
            if (CollUtil.isEmpty(skuList)) {
                continue;
            }
            spuDAO.updateById(buildSpuStatistics(id, skuList));
        }
    }

    /**
     * 按 SKU 列表聚合 SPU 冗余统计列
     *
     * <p>口径与 {@code SpuDomainImpl.calculateSpuStatistics} 对齐: 价格区间取
     * min/max, 库存取求和, 最大利润取 max(市场价 - 销售价)。不触碰 minPricingNum
     * 等与 SKU 聚合无关的列。</p>
     *
     * @param spuId   SPU 主键
     * @param skuList 该 SPU 下的 SKU 列表, 非空
     * @return 仅含待刷新列与主键的 SpuDO
     */
    private SpuDO buildSpuStatistics(Long spuId, List<SkuDO> skuList) {
        ...
        spuDO.setId(spuId);
        spuDO.setMarketPrice(minMarket);
        spuDO.setMarketPriceBegan(minMarket);
        spuDO.setMarketPriceEnd(maxMarket);
        spuDO.setSupplyPrice(minSupply);
        spuDO.setSupplierPriceBegan(minSupply);
        spuDO.setSupplierPriceEnd(maxSupply);
        spuDO.setSalePrice(minSale);
        spuDO.setSalePriceBegan(minSale);
        spuDO.setSalePriceEnd(maxSale);
        spuDO.setUnitPrice(minUnit);
        spuDO.setMaxProfit(maxProfit);
        spuDO.setInventory(inventory);
        return spuDO;
    }
