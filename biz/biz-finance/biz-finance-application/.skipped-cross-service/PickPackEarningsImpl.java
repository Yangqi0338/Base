...
import com.zkl.scm.finance.model.earnings.req.EarningsPackExecReq;
import com.zkl.scm.finance.model.earnings.vo.PackOrderRpcVO;
import com.zkl.scm.finance.model.earnings.vo.PickPackInfoVO;
import com.zkl.scm.infrastructure.redis.aspect.DistributedLock;
...
import com.zkl.scm.user.model.relation.req.UserTaskQuery;
import com.zkl.scm.user.model.relation.res.UserTaskRes;
import com.zkl.scm.user.model.relation.vo.PermissionRpcVO;
import com.zkl.scm.user.model.relation.vo.UserTaskVO;
import com.zkl.scm.user.rpc.facade.ILevelFacade;
import com.zkl.scm.user.rpc.facade.UserTaskFacade;
 ...
 */
@Service
public class PickPackEarningsImpl extends EarningStrategySupport implements IConsumeEarnings<EarningsPackExecReq> {
    ...
    public List<EarningRecordReq> earnings(EarningsPackExecReq req, List<AlterAccountContributeDataReq> contributeDataReqs) {
        ...
        Integer amount = packOrder.getAmount();
        ...
        PermissionRpcVO permissionVO = levelFacade.levelPermissionVO(directRole, 1);
        ...
        Integer shareAmount = ScmUtil.percentFloor(amount, ratio);
        if (shareAmount < 1) {
            return earningInfos;
        }
...
//        earningRecordReq.setContributeName("");
    ...
    public EarningRecordReq buildEarningsByInviteId(Long accountId, RoleEnum.CompanyRole role, EarningsPackExecReq req, Integer amount) {
        EarningsEnum.EarningType earningType = EarningsEnum.EarningType.getByRole(consumeType(), role);
        if (earningType == null) {
            throw new ScmException(BaseErrorCode.PARAM.getCode(), "不可用的分润类型");
        }
