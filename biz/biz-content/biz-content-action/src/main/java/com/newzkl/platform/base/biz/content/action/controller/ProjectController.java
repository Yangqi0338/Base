...
import com.newzkl.platform.base.biz.content.domain.service.ProjectDomain;
 ...
 * 平台-项目控制器
 *
 * <p>迁移说明: 由 biz-sys 迁入 biz-content。省/市/区名称回填改经本域出站端口
 * {@code RegionApi} 调 biz-sys 区域能力(经 {@code RegionFacade}), 返回轻量
 * {@code RegionSimpleVO}(code/pid/name), 不再直接依赖 biz-sys 域的 {@code RegionDomain}/{@code Area}。</p>
 *
 ...
 * @author KC
 */
@RestController("contentProjectController")
@RequestMapping("/project")
@RequiredArgsConstructor
@FuncPermission("平台-项目")
public class ProjectController {
    ...
    private List<ProjectListRes> fillRegionName(List<ProjectListRes> resList) {
        ...
        List<RegionSimpleVO> areaList = regionApi.getRegionList(null, true);
        resList.forEach(res -> {
     ...
     * @return 区域名称, 未命中返回空串
     */
    private String findNameByCode(List<RegionSimpleVO> areaList, Integer parentCode, Integer code) {
        if (CollUtil.isEmpty(areaList)) {
            if (code == null || parentCode == null) {
                return "";
            }
            areaList = regionApi.getRegionList(parentCode, false);
        }
        if (code == null || parentCode == null) {
            return "";
        }
        return areaList.stream()
                ...
                .findFirst().map(RegionSimpleVO::getName).orElse("");
    }
}
