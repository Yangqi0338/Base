ALTER TABLE
  `model_shop_order_record` MODIFY COLUMN `id` bigint NOT NULL COMMENT '主键ID',
  MODIFY COLUMN `store_id` bigint NULL COMMENT '门店id',
  MODIFY COLUMN `type` varchar(255) NULL COMMENT '订单类型' AFTER `model_shop_id`,
  MODIFY COLUMN `amount` bigint NULL COMMENT '金额' AFTER `type`,
ADD
  COLUMN `executor` json NULL COMMENT '操作人信息' AFTER `amount`,
ADD
  COLUMN `creator_id` bigint NULL COMMENT '创建人id' AFTER `executor`,
  MODIFY COLUMN `create_time` datetime NULL COMMENT '创建时间',
ADD
  COLUMN `update_time` datetime NULL COMMENT '更新时间',
ADD
  COLUMN `del_flag` int NULL DEFAULT 0 COMMENT '逻辑删除标记(正常 0, 删除为 NULL(确保唯一索引生效))',
ADD
  INDEX `auto_idx_model_shop_order_record_store_id`(`store_id`) COMMENT '门店id',
ADD
  INDEX `auto_idx_model_shop_order_record_model_shop_id`(`model_shop_id`) COMMENT '样板店ID',
  COMMENT = '样板店订单记录(DB 表 model_shop_order_record)';
